...
// Klaus ist ein Sprinter: Bei jedem sicheren Sprung wird die Sprint-Variante
// bevorzugt, damit er mit möglichst viel Tempo zum Ziel kommt.
const klausNavigator = createNavigator({
    sprint: true,
    hazardLookahead: 280,
    gapLookahead: 260,
    combinedLookahead: 300,
    choosePlan(context, plans) {
        const sawAhead = context.problems.hazards.some(
            (hazard) => hazard.kind === "schnetzler"
        );
        if (sawAhead) {
            // Sägen sind nicht betretbar: lieber einen hohen, weiten Sprung wählen
            // als den kürzesten Sprung, der beim Landen auf der Säge enden kann.
            return [...plans]
                .filter((plan) => plan.sprint)
                .sort((a, b) => b.holdJumpTicks - a.holdJumpTicks)[0] || null;
        }
        // Für Lücken braucht Klaus den Sprintsprung; sonst nimmt er den kürzesten
        // sicheren Sprung, damit er nicht unnötig weit fliegt.
        if (context.problems.gapNear) {
            return plans.find((plan) => plan.sprint) || plans[0] || null;
        }
        return plans[0] || null;
    },
    recoverFromStuck(context) {
        const mustWait = context.state.hazards.some(
            (hazard) =>
                hazard.dx * context.direction > 0 &&
                Math.abs(hazard.dx) < 180 &&
                Math.abs(hazard.dy) < 72 &&
                ((hazard.kind === "loderix" && hazard.active) ||
                    (hazard.kind === "spikehead" && (hazard.active || hazard.warning)))
        );
        // Feuer und fallende Stachelköpfe abwarten; bei allen anderen Blockaden
        // nach vorn springen, damit Klaus nicht in einer Endlosschleife wartet.
        return mustWait
            ? ["idle"]
            : ["jump", moveToward(context.direction, true)];
    },
});

let boingoJumpTicks = 0;
let waitingForBoingoBoost = false;
...
export default {
    ...
    author: "Krise",
    decide(state) {
        if (waitingForBoingoBoost) {
            // Beim Ausrichten darf keine andere Strategie seitlich übernehmen.
            if (state.velocity.vy < -700) {
                waitingForBoingoBoost = false;
                boingoJumpTicks = 0;
            } else if (boingoJumpTicks > 0) {
                boingoJumpTicks--;
                return ["jump"];
        ...
        const direction = state.goalDirection.dx >= 0 ? 1 : -1;
        const activeFireAhead = state.hazards.find(
            (hazard) =>
                hazard.kind === "loderix" &&
                hazard.active &&
                hazard.dx * direction > 0 &&
                Math.abs(hazard.dx) < 240 &&
                Math.abs(hazard.dy) < 72
        );
        const activeFireBehind = state.hazards.some(
            (hazard) =>
                hazard.kind === "loderix" &&
                hazard.active &&
                hazard.dx * direction < 0 &&
                Math.abs(hazard.dx) < 120 &&
                Math.abs(hazard.dy) < 72
        );
        const dangerousSpikeheadAhead = state.hazards.some(
            (hazard) =>
        ...
        const boingoAhead = state.utilities.find(
            (utility) =>
                utility.kind === "boingo" &&
                utility.dx * direction >= 0 &&
                Math.abs(utility.dx) < 200 &&
                Math.abs(utility.dy) < 72
        );
        const raisedPlatformVisible = state.platforms.some(
            (platform) =>
                platform.kind !== "ceiling" &&
                platform.dy < -state.tuning.tileSize &&
                platform.dx * direction >= -state.tuning.botWidth
        );

        // Mit Sicherheitsabstand warten; direkt vor dem Feuer lieber abspringen.
        if (state.onGround && activeFireAhead) {
            if (Math.abs(activeFireAhead.dx) <= state.tuning.botWidth * 2) {
                return ["jump", moveToward(direction, true)];
            }
            return ["idle"];
        }

        // Zange aus Feuer hinten und fallendem Spikehead vorn: warten, bis eine
        // Seite wieder ungefährlich ist. Vorwärtsrennen wäre hier tödlich.
        if (state.onGround && activeFireBehind && dangerousSpikeheadAhead) {
            return ["idle"];
        }

        // Der Navigator kann für einen Anlauf rückwärts steuern. Brennt hinter
        // Klaus Feuer, hat Weglaufen nach vorn Vorrang vor diesem Rückwärts-Timer.
        if (state.onGround && activeFireBehind) {
            return [moveToward(direction, true)];
        }

        // Ein Boingo ist hier der einzige Weg nach oben: erst genau ausrichten,
        // dann senkrecht darauf springen und beim Herunterfallen auslösen.
        if (state.onGround && boingoAhead && raisedPlatformVisible) {
            if (Math.abs(boingoAhead.dx) <= state.tuning.botWidth * 1.5) {
                waitingForBoingoBoost = true;
                boingoJumpTicks = Math.ceil(state.tuning.minJumpHoldMs / state.tuning.tickMs);
                return ["jump"];
            }
            return [moveToward(boingoAhead.dx, false)];
        }

        return klausNavigator.decide(state);
    },
};

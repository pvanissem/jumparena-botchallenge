...
// Merkt sich über mehrere Ticks, wie lange "jump" noch gehalten werden soll
// (siehe createJumpHold weiter oben in dieser Datei).
let masterMikeJumpHold = createJumpHold();
// Zaehlt, wie lange schon vor einer aktiven Loderix (Feuer) gewartet wird -
// Sicherheitsgrenze, damit nie unbegrenzt "idle" zurückgegeben wird.
let masterMikeLoderixWaitTicks = 0;
const MASTER_MIKE_MAX_LODERIX_WAIT_TICKS = 60; // ~2s, mehr als ein voller An/Aus-Zyklus
...
export default {
  ...
  // Strategie: schnell + vorsichtig.
  // - Ninjafrogs werden aktiv angesprungen (stompen = sicherer Kill).
  // - Boingos werden IMMER gezielt angesteuert und aktiv angesprungen.
  // - Vor Abgründen/Gefahren wird mit genug Schwung (Sprint) drübergesprungen.
  // - Nahe Früchte werden im Vorbeilaufen mitgenommen, ohne grossen Umweg.
  // - Alle anderen (nicht-stompbaren) aktiven Gefahren werden umsprungen.
  decide(state) {
    if (state.justRespawned) {
      masterMikeJumpHold = createJumpHold();
      masterMikeLoderixWaitTicks = 0;
    }

    const goalDir = state.goalDirection.dx >= 0 ? 1 : -1;
    let moveDir = goalDir;
    const wantSprint = true;

    // Nächster Ninjafrog in Reichweite? -> gezielt draufspringen (stompen).
    const froggy = state.hazards.find(
      (h) => h.kind === "ninjafrog" && h.stompable && Math.abs(h.dx) < 90 && Math.abs(h.dy) < 70
    );

    // Nächste echte Gefahr (nicht stompbar) voraus, die gerade aktiv ist oder
    // sich ankündigt (spikehead-warning)?
    const danger = state.hazards.find(
      (h) =>
        !h.stompable &&
        (h.active || h.warning) &&
        h.dx * goalDir > 0 &&
    ...
    // Abgrund in Laufrichtung nah genug, um jetzt schon zu planen?
    const gapNear = state.gapAhead.present && state.gapAhead.distance < 140;

    // Unspringbare Wand im Weg?
    const wall = wallAhead(state);

    let jumpWanted = false;
    let jumpHoldTicks = 12;

    if (state.onGround && (danger || gapNear || wall)) {
      // Mehrere Sprunghaltezeiten mit Sprint durchprobieren, den kürzesten
      // sicheren Plan nehmen: Landung muss auf festem Boden sein, hinter dem
      // ganzen Hindernis liegen und (falls Gefahr) diese sicher überfliegen.
      const holdOptions = [6, 8, 10, 12, 16, 20];
      let chosen = null;
      for (const hold of holdOptions) {
        ...
        const clearsGap =
          !gapNear ||
          landing.dx * goalDir > state.gapAhead.distance + state.tuning.botWidth;
        const clearsWall = !wall || landing.dx * goalDir > wall.distance + state.tuning.botWidth;

        if (clearsDanger && clearsGap && clearsWall) {
          chosen = hold;
          break;
        }
      }
      if (chosen !== null) {
        jumpWanted = true;
        jumpHoldTicks = chosen;
      }
    }

    // Kein sicherer Sprung über eine gerade AKTIVE Loderix gefunden? Dann
    // NICHT blind reinlaufen, sondern stehen bleiben und warten, bis sie
    // wieder ausgeht (getaktet ~1,5s an/aus - siehe active-Feld). Begrenzt
    // durch MASTER_MIKE_MAX_LODERIX_WAIT_TICKS, damit nie unbegrenzt gewartet
    // wird, falls die Erkennung mal danebenliegt.
    const activeLoderixBlocking = danger && danger.kind === "loderix" && danger.active;
    if (
      !jumpWanted &&
      activeLoderixBlocking &&
      masterMikeLoderixWaitTicks < MASTER_MIKE_MAX_LODERIX_WAIT_TICKS
    ) {
      masterMikeLoderixWaitTicks++;
      const waitActions = [];
      // Einen bereits laufenden Sprung sauber zu Ende bringen, falls einer
      // gerade aktiv war.
      if (masterMikeJumpHold.tick(false, 0)) waitActions.push("jump");
      waitActions.push("idle");
      return waitActions;
    }
    masterMikeLoderixWaitTicks = 0;

    // Wenn kein Sprung wegen Gefahr/Lücke/Wand nötig ist: Ninjafrog gezielt
    // ansteuern und draufspringen (kostet kein Leben, gibt Extra-Punkte-Gefühl
    // fürs Publikum). Sicherheit geht aber vor - siehe Bedingung oben.
    if (!jumpWanted && froggy && state.onGround) {
      const hold = minJumpHoldToReach(
        state,
        froggy.dx,
        froggy.dy,
        state.tuning.botWidth * 0.6,
        { dir: froggy.dx >= 0 ? 1 : -1, sprint: true }
      );
      if (hold !== null) {
        moveDir = froggy.dx >= 0 ? 1 : -1;
        jumpWanted = true;
        jumpHoldTicks = hold;
      }
    }

    // Boingo (Trampolin) in Reichweite? -> IMMER gezielt ansteuern und aktiv
    // draufspringen (nicht nur zufällig durchlaufen), damit der kräftige
    // Boost sicher mitgenommen wird.
    if (!jumpWanted) {
      const boingo = state.utilities.find(
        (u) => u.kind === "boingo" && Math.abs(u.dx) < 110 && Math.abs(u.dy) < 90
      );
      if (boingo) {
        if (state.onGround) {
          const hold = minJumpHoldToReach(
            ...
            boingo.dy,
            state.tuning.botWidth * 0.6,
            { dir: boingo.dx >= 0 ? 1 : -1, sprint: true }
          );
          if (hold !== null) {
            moveDir = boingo.dx >= 0 ? 1 : -1;
            jumpWanted = true;
            jumpHoldTicks = hold;
          } else {
            // Kein separater Sprung nötig (z.B. Boingo liegt direkt am Weg) -
            // trotzdem gezielt draufsteuern statt daran vorbeizulaufen.
            moveDir = boingo.dx >= 0 ? 1 : -1;
          }
        } else {
          // In der Luft: Flugrichtung so korrigieren, dass die Landung auf
          // dem Boingo trifft.
          moveDir = boingo.dx >= 0 ? 1 : -1;
        }
      }
    ...
    // Sonst: nahegelegene Frucht im Vorbeilaufen mitnehmen (kein grosser
    // Umweg - nur ein leichter Schlenker).
    if (!jumpWanted) {
      const coin = state.nearestCoin;
      const coinClose = coin && Math.abs(coin.dx) < 70 && Math.abs(coin.dy) < 50;
      if (coinClose) {
        moveDir = coin.dx === 0 ? goalDir : coin.dx > 0 ? 1 : -1;
      }
    }

    const actions = [];
    if (masterMikeJumpHold.tick(jumpWanted, jumpHoldTicks)) actions.push("jump");
    actions.push(moveToward(moveDir, wantSprint));
    return actions;
  },
};

...
const paths = [
  ...
  "examples/strategies/visitor-builder.js",
];
...
describe("v2 visitor-owned strategy artifacts", () => {
  ...
  it.each([48, 1000])(
    ...
    (width) => {
      ...
      expect(observe(load("examples/strategies/visitor-builder.js"), input).command).toMatchObject({
        kind: "jump",
        platformId: "next",
        x: 200,
      });
  ...
  it("collector chooses a nearby fruit until the endspurt while sprinter keeps the goal", () => {
    ...
    expect(observe(load("examples/strategies/collector.js"), input).command).toMatchObject({
      kind: "walk",
      x: 152,
    });
    expect(observe(load("examples/strategies/sprinter.js"), input).command).toMatchObject({
      kind: "walk",
      x: 950,
    });
    expect(
      observe(load("examples/strategies/collector.js"), { ...input, timeElapsedMs: 65000 }).command
  ...
  it("cautious waits when a nearby active hazard blocks its direction", () => {
    ...
    expect(observe(load("examples/strategies/cautious.js"), input)).toEqual({
      command: null,
      result: [],
    });
  });
...
describe.each(["examples/strategies/messe-demo.js"])(
  "explicit Level 1 route: %s",
  (path) => {
    ...
    it("uses the first spring to stay above the two fires until a clear landing", () => {
      ...
      expect(
        observe(bot, input, { state: "succeeded", commandId: first?.id }).command
      ).toMatchObject({
        kind: "boingo",
        utilityId: "boingo-1",
...
describe("fresh visitor template", () => {
  it("returns no actions and starts no movement, including after respawn", () => {
    ...
    for (const input of [state(), passage(), { ...state(), justRespawned: true }]) {
      expect(observe(bot, input)).toEqual({ command: null, result: [] });
    }
  });
});

...
- [ ] **A1. Message-Contracts + Typguards** (US-1, US-3, US-4, US-6;
...
- [ ] **B1. `BotRegistry`** (US-1, US-4; Design "Server: botRegistry/")
...
- [ ] **B3. Persistenz-Port + JSON-Datei-Adapter** (US-5)
...
- [ ] **C1. `createBotAddHandler`** (US-1, US-2, US-5; Design "Validierung")
...
- [ ] **C2. `createBotRemoveHandler`** (US-4, US-5)
...
- [ ] **D1. `parseInboundMessage` erweitern** (US-1, US-4)
...
- [ ] **D2. Broadcast an alle inkl. Sender** (US-3; Design "Warum routeToAll")
...
- [ ] **D3. Snapshot für neu verbundene Clients** (US-3)
...
- [ ] **D4. Composition Root verdrahten** (US-1, US-3, US-4, US-5)
...
- [ ] **E1. Worker meldet Modul-Metadaten** (US-2; Design "Protokoll-Erweiterung")
...
- [ ] **E2. `BotRunner` bleibt unverändert im Verhalten** (Regressionsschutz)
...
- [ ] **E3. `validateBotArtifact`** (US-1, US-2)
...
- [ ] **F4. `/admin` und `/present` anbinden** (US-1, US-3, US-4)
...
- [ ] **G3. Gesamtabgleich gegen `requirements.md`**
